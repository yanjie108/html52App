...
(function($) {
  ...
  */
  ...
  /*
    ...
    @return object css
  */
  fi.prototype.parseCSS = function(source,rectframesjonstr) {
    ...
    var rectframes=null;
    if(rectframesjonstr){
      rectframes=JSON.parse(rectframesjonstr);
    }
    debuginfo="";
    var newcssnames=[];
    for(var one in css){
      var onecss=css[one];
      if(onecss.type=="media"){
        if(rectframes==null)continue;
        var selector=onecss.selector;
        var subStyles=onecss.subStyles||[]; 
        var subStyleslen=subStyles.length;
        if(subStyleslen>0){
          let pattern =/\((.+?)\)/g;
          var res = selector.match(pattern); 
          var reslen=res.length;
          if(reslen<1){
            continue;
          }
          var begin=parseInt(res[0].substring(1,res[0].length-1).split(":")[1]);
          var end=0;
          if(reslen>1){
            end=parseInt(res[1].substring(1,res[1].length-1).split(":")[1]);
          }
          if((rectframes.width>=begin && end<1) || (rectframes.width>=begin && rectframes.width<=end)){
            newcssnames=newcssnames.concat(subStyles);
          }
        }
      }else{
        newcssnames.push(onecss);
      }
    } 
    return newcssnames;
  };
  fi.prototype.getdebugstr = function() {
    return debuginfo;
  }
  /*
  ...
  */
  ...
  fi.prototype.test1 = function(arg1) {
    return arg1;
  }
